...
export default function Marquee() {
  ...
  return (
    <>
      ...
      `}</style>
      <div style={{ width:'100%', overflow:'hidden', background:'var(--cream-d)', borderBottom:'1px solid var(--cream-b)', height:260 }}>
        <div className="mq-track" style={{ display:'flex', gap:3, height:'100%', width:'max-content' }}>
          {all.map((src, i) => (
            <div key={i} className="mq-item" style={{ flexShrink:0, width:190, height:260, overflow:'hidden' }}>
              <img src={src} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', objectPosition:'top center', display:'block', transition:'transform .5s' }} />
            </div>
          ))}
        </div>
      </div>
